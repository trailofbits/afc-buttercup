...
import string
import subprocess
from buttercup.common.clusterfuzz_parser.slice import *
from buttercup.common.clusterfuzz_parser import utils
from buttercup.common.clusterfuzz_parser import crash_analyzer
...
class StackParser:
  ...
  def remove_lkl_kernel_times_and_set_params(self, state, crash_data):
    ...
    for line in crash_data.splitlines():
      ...
      if not state.lkl_kernel_build_id:
        match = LINUX_VERSION_REGEX_LKL.match(clean_line)
        if match:
  ...
  def parse(self, stacktrace: str) -> CrashInfo:
    ...
    state.is_lkl = LINUX_KERNEL_MODULE_STACK_TRACE in stacktrace
    ...
    for line in split_crash_stacktrace:
      ...
      self.update_state_on_match(
          EXTRA_SANITIZERS_PYSECSAN, line, state, new_type='PySecSan')
